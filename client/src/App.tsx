...
import { Calculator, Database, Menu } from "lucide-react";
...
const App = () => {
  ...
  return (
    <>
      {/* Topbar */}
      <div className="fixed h-14 z-40 left-0 shadow-sm bg-white inset-x-0 flex items-center px-2">
        <div className="flex gap-2 items-end">
          <button
            className="cursor-pointer active:scale-90 p-1 bg-slate-100 rounded"
            onClick={() => setMobileSidebarOpen(!mobileSidebarOpen)}
          >
            <Menu size={24} />
          </button>
          <h1 className="text-xl font-medium">EStock</h1>
        </div>
      </div>
      {/* End of Topbar */}
      ...
      <div className="flex">
        ...
        {/* Sidebar */}
        <div
          className={`w-64 p-3 mt-14 h-screen bg-white shadow fixed z-20 ${mobileSidebarOpen ? " md:translate-x-0" : "md:translate-x-0 -translate-x-full"} transition-transform`}
        >
          <h1>Menu</h1>
          <div className="mt-5 flex flex-col gap-1">
            <a
              href="#"
              className="py-2 px-3 flex items-center gap-1 rounded-md hover:text-white hover:bg-red-500 transition-colors"
            >
              <Database size={18} />
